...
import { splitStyleProps, CSSProps, CSSSelectorProps } from './props';
...
> = Props extends { as: infer NodeName }
  ? { as: NodeName } & Partial<
      JSX.IntrinsicElements[NodeName & keyof JSX.IntrinsicElements]
    > &
      Partial<CSSProps> &
      Partial<Record<CSSSelectorProps, Partial<CSSProps>>> &
      ExtraProps
  : Partial<JSX.IntrinsicElements[DefaultNode]> &
      Partial<CSSProps> &
      Partial<Record<CSSSelectorProps, Partial<CSSProps>>> &
      ExtraProps;
...
> = <Props>(
  props: Props & AsNodeProps<Props, DefaultNode, ExtraProps>
) => JSX.Element;

const tags: Partial<
  Record<keyof JSX.IntrinsicElements, Tagged<keyof JSX.IntrinsicElements>>
> = {};

export const Box: StyledComponent = (props) => {
  const splittedProps = splitStyleProps(props, 'div');
  const tag =
    tags[splittedProps.as] ||
    (tags[splittedProps.as] = styled(splittedProps.as));
